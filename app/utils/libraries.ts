 ...
 *
 * `status`: `docs` has its own site, `npm` is published without one, `soon` has nothing
 * on npm yet. Only `docs` and `npm` rows get links, and a GitHub link needs `repo` too:
 * a package can be published while its repository stays private.
 */
...
export interface LibraryInfo {
  ...
  /** Public GitHub repository. A package can be on npm while its repo stays private. */
...
export const LIBRARIES: readonly LibraryInfo[] = [
  define({
    key: "web",
    description: "Web search, reverse image search and page reading over Brave, Exa, Tavily, Firecrawl, Jina, SearXNG and six more.",
    group: "web",
    status: "docs",
    icon: "i-lucide-globe",
    color: "teal",
    ...
    repo: true,
    providers: ["Brave", "Exa", "Tavily", "Firecrawl", "Jina", "SearXNG", "Mojeek", "SerpAPI", "SerpBase", "TinyFish", "Context", "OpenAI Codex"],
  ...
  define({
    ...
    color: "orange",
    accent: "#fdba74",
    site: "https://harnesses.agntn.dev",
    repo: true,
    providers: ["Claude Code", "Codex", "Gemini CLI", "Cursor", "GitHub Copilot", "OpenCode", "Grok", "Antigravity", "Pi", "OMP", "MastraCode", "Freebuff", "Prime Agent"],
    surfaces: ["library", "cli", "mcp", "pi", "omp"],
  }),
  define({
    key: "explorers",
  ...
  define({
    ...
    color: "fuchsia",
    accent: "#f0abfc",
    site: "https://ciphers.agntn.dev",
    repo: true,
    providers: ["Caesar", "Vigenère", "Playfair", "Enigma", "ADFGVX", "Bifid", "Polybius", "Bacon", "Morse", "Rail fence", "+10"],
    surfaces: ["library", "cli", "mcp", "pi", "omp"],
  }),
  define({
    key: "puzzles",
  ...
  define({
    ...
    site: null,
    repo: false,
    providers: ["Claude Code", "Codex", "Gemini CLI", "Pi", "OMP", "AEI", "Mastra", "OpenClaw", "OpenCode"],
    surfaces: ["library", "cli", "mcp", "pi", "omp"],
  }),
  define({
    key: "memory",
  ...
  define({
    ...
    site: null,
    repo: false,
    providers: ["PublicNode", "Forward Research", "Custom endpoints"],
    surfaces: ["library", "cli", "mcp", "pi", "omp"],
  }),
  define({
    key: "nfts",
...
export const GROUPS: readonly { readonly key: LibraryGroup; readonly label: string; readonly blurb: string }[] = [
  ...
  { key: "chain", label: "Chains", blurb: "Keys, nodes, explorers, puzzles and NFT data." },
